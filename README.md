...
- 한글 텍스트 시각화 시 폰트 미설정으로 인한 글자 깨짐 문제를 `malgun.ttf` 폰트 지정으로 해결
- 형태소 분석 결과에서 의미 없는 단어(조사, 대명사 등)를 걸러내기 위한 불용어 사전을 직접 구성
- Streamlit의 `st.pyplot()`을 활용하여 Matplotlib 그래프를 웹에서 바로 렌더링

---

## 📌 향후 개선 아이디어

- [ ] 사용자가 직접 CSV 파일을 업로드할 수 있는 기능 추가
- [ ] 불용어 리스트를 사용자가 직접 편집할 수 있는 UI 제공
- [ ] 감성 분석(긍정/부정) 기능 결합
...
- Repo: [GitHub Repository 링크]

## 👤 Author

- 이름 / 연락처 / 포트폴리오 링크 등을 여기에 추가해보세요.
